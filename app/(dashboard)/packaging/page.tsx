...
import { packagingTypeLabel, PACKAGING_TYPE_COLOURS, PACKAGING_TYPES } from '@/lib/constants'
...
interface PageProps { searchParams: { type?: string; supplier?: string } }

export default async function PackagingPage({ searchParams }: PageProps) {
  const supabase = createClient()

  const [{ data: rows }, { data: suppliers }, { data: balances }] = await Promise.all([
  ...
  return (
    <div className="space-y-5">
      ...
      <form className="bg-white border border-gray-200 rounded-lg p-3 flex items-center gap-2 text-xs" action="/packaging" method="get">
