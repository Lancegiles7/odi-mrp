...
import { purgeExpiredIngredients, restoreIngredient } from '../actions'
import { PurgeIngredientButton } from '@/components/ingredients/purge-ingredient-button'

export const metadata: Metadata = { title: 'Ingredient trash' }

interface PageProps {
  searchParams: { purged?: string; error?: string }
}

export default async function IngredientTrashPage({ searchParams }: PageProps) {
  ...
  if (profile?.roles?.name !== ROLES.ADMIN) {
    return (
      <div className="max-w-xl">
        <h1 className="text-2xl font-semibold">Ingredient trash</h1>
  ...
  const { purged } = await purgeExpiredIngredients()

  const { data: rows } = await supabase
    .from('ingredients')
    .select('id, sku_code, name, deleted_at, deleted_by, user_profiles:deleted_by(full_name)')
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex items-start justify-between mb-6">
        <div>
          <Link href="/ingredients" className="text-sm text-gray-500 hover:text-gray-900">← Ingredients</Link>
          <h1 className="text-2xl font-semibold text-gray-900 mt-2">Trash</h1>
          <p className="text-sm text-gray-500 mt-1">
            Soft-deleted ingredients. Automatically purged after {SOFT_DELETE_WINDOW_DAYS} days. Admin only.
          </p>
        </div>
      </div>
      ...
      {searchParams.purged && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-md text-sm text-green-700">
          Ingredient permanently deleted.
        </div>
      )}
      {searchParams.error && (
        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md text-sm text-red-700">
          {searchParams.error === 'restore_failed'
            ? 'Could not restore — please try again.'
            : 'Could not permanently delete — the ingredient is still referenced by a recipe, PO line or stock balance. Restore it, clear those references first, then delete again.'}
        </div>
      )}
      {purged > 0 && (
                  ...
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex gap-2 justify-end">
                        <form action={restoreIngredient.bind(null, row.id)}>
                          <button
                            type="submit"
                        ...
                        <PurgeIngredientButton ingredientId={row.id} ingredientName={row.name} />
                      </div>
                    </td>
                  </tr>
                )
