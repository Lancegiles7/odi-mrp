...
/**
 * Per-month "units short" (i.e. demand minus what's available from
 * carried stock + this month's PO arrivals). Sum across months gives
 * the total unmet demand for the rolling year.
 */
export function monthShortAmounts(row: PackagingRow, opening: number, months: string[]): Map<string, number> {
 ...
 * sub-label on the Packaging demand row.
 */
export function monthRunningBalances(row: PackagingRow, opening: number, months: string[]): Map<string, number> {
  const out = new Map<string, number>()
  let balance = opening
  for (const m of months) {
    const demand   = row.demandByMonth.get(m)   ?? 0
