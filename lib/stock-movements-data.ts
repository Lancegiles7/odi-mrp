/**
 * Shared loader for the Stock Movements ledger.
 *
 * Assembles every per-country/per-product/per-month map from the DB (receipts,
 * BvA actuals, write-offs, production plan, demand, open POs) and rolls them
 * into the stacked NZ/AUS ledger via buildStockLedger. Both the Stock Movements
 * page and the Production page use this so the closing (EOM) figures always
 * agree — Stock Movements is the single source of truth for stock on hand.
 */
...
export async function loadStockLedger(): Promise<StockLedger> {
  ...
  // month taking precedence (per country). No make-to-demand default.
...
/**
 * Per-product closing stock (EOM) at a given month, split by market. Used by
 * Production to default each row's opening stock to the last closed month's EOM.
 */
export function closingStockAt(rows: StockRow[], month: string | null): Map<string, { NZ: number; AU: number }> {
  const out = new Map<string, { NZ: number; AU: number }>()
  if (!month) return out
  for (const r of rows) {
    const eom = r.actual[month]?.eom ?? 0
    const e = out.get(r.product_id) ?? { NZ: 0, AU: 0 }
    e[r.market] = eom
    out.set(r.product_id, e)
  }
  return out
}
