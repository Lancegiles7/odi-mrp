...
export function calcProductCostSummary(
  ...
  // The AUD build up to the factory gate — everything except freight. For an
  // AU-made / dual product this is VMC's real build (AU ingredient landed costs
  // + VMC toll + AU packaging); for a plain NZ product it's the NZ build
  // converted. Freight is country-specific, so each market total adds its own.
  // Left unrounded so the AU total rounds once, exactly as it always has.
  const auExFreight =
    auIngredientTotal +
  ...
  // AU total — every line expressed in AUD, plus freight within Australia.
  const auGrandTotal = round2(auExFreight + toAud(freightAu, freightAuCur))

  // NZ total (NZD). An AU-made product is manufactured at VMC and shipped here,
  // so its NZD cost is the real AUD build converted at FX plus freight to NZ —
  // NOT the AUD grand total × FX, which would carry freight within Australia
  // and drop the cost of getting the stock across the Tasman. Otherwise it's
  // the Brand Nation NZ build (every line expressed in NZD).
  const nzGrandTotal = auMade
    ? round2(auExFreight * fxRate + toNzd(freightNz, freightNzCur))
    : round2(
        ingredientTotal +
        packaging +
        toNzd(toll,      tollCur) +
