  ...
  return (
    <>
      <tr className="border-t border-gray-100 hover:bg-gray-50/50 cursor-pointer" onClick={() => setOpen((v) => !v)}>
        ...
        <td className={`px-3 py-2 text-right tabular-nums font-semibold border-l ${shortByMonth.get(months[months.length - 1]) ? 'bg-red-100 text-red-800 border-red-200' : 'bg-gray-50 text-gray-900 border-gray-200'}`}>
          {fmt(row.totalDemand)}
        </td>
      </tr>
      ...
      {hasArrivals && (
        <tr className="border-t border-blue-100 bg-blue-50/30 text-[11px]">
          ...
          {months.map((m) => {
            ...
            return (
              <td key={m} title={title} className={`px-2 py-1.5 text-right tabular-nums border-l border-blue-100/60 ${v > 0 ? 'text-blue-700 font-semibold' : 'text-gray-300'}`}>
                {v > 0 ? `+${fmt(v)}` : '—'}
              </td>
            )
          })}
